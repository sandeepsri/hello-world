pipeline {
  ...
  stages {
   ...
   //   steps {
   //     sh 'scp -o StrictHostKeyChecking=no target/*.war sandeep@192.168.0.7:/home/sandeep/tomcat9/webapps/'
   //   }
   // }
    stage ('DAST') {
      steps {
        sh 'docker run owasp/zap2docker-stable zap-baseline.py -d -t http://192.168.0.7:8083/webapp'
      }
    }
  }
}
